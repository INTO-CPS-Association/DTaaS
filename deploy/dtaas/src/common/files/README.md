...
- `files/user1/` and `files/user2/` are mounted as `/workspace`
  in server packages.
- `files/template/` provides a starter folder structure
  for creating new users in server packages.

For localhost packages, create additional user folders
by copying `files/user1/`.
