...
def mock_service_setup():
    ...
    with patch("dtaas_services.cmd.Service") as mock_service_class, patch(
        "dtaas_services.cmd.copy_certs"
    ) as mock_copy_certs, patch(
        "dtaas_services.cmd.permissions_mongodb"
...
def test_setup_config_not_found(runner):
    ...
    with patch(
        "dtaas_services.pkg.utils.os.geteuid", return_value=0, create=True
    ):  # Pretend to be root
        with patch(
