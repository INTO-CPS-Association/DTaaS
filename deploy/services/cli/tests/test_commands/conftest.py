...
def mock_service_setup(mocker):
    """Mock Service class and host setup functions"""
    ...
    mock_copy_certs = mocker.patch("dtaas_services.commands.host_ops.copy_certs")
    mock_mongodb = mocker.patch("dtaas_services.commands.host_ops.permissions_mongodb")
    mock_influxdb = mocker.patch(
        "dtaas_services.commands.host_ops.permissions_influxdb"
    )
    mock_rabbitmq = mocker.patch(
        "dtaas_services.commands.host_ops.permissions_rabbitmq"
    )
    mock_thingsboard = mocker.patch(
        "dtaas_services.commands.host_ops.permissions_thingsboard"
    )
    mocker.patch("dtaas_services.commands.host_ops.check_root_unix")
