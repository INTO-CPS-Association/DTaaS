...
test.describe('Digital Twin Execution Log Cleaning', () => {
  ...
  test('Execute Digital Twin and verify log cleaning', async ({ page }) => {
    ...
    const helloWorldCard = page.locator('.MuiPaper-root:has-text("Hello world")').first();
    await expect(helloWorldCard).toBeVisible({ timeout: 10000 });
    
    const startButton = helloWorldCard.locator('button:has-text("Start")');
    await startButton.click();
    
    await expect(helloWorldCard.locator('button:has-text("Stop")')).toBeVisible({ timeout: 15000 });
    
    await expect(helloWorldCard.locator('button:has-text("Start")')).toBeVisible({ timeout: 90000 });
    
    const logButton = helloWorldCard.locator('button:has-text("LOG"), button:has-text("Log")');
    ...
    const logContent = await logDialog.locator('div').filter({ hasText: /Running with gitlab-runner/ }).first().textContent();
    ...
    if (logContent) {
      ...
      // eslint-disable-next-line no-control-regex
      expect(logContent).not.toMatch(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/);
