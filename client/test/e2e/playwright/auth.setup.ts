...
const testURI = process.env.REACT_APP_URL ?? '';
...
setup('authenticate', async ({ page }) => {
    ...
    await page.route('**/*', (route, request) => {
      if (request.url().includes(testURI.toString())) {
        // Add Basic Auth headers only for requests to your application
        route.continue({
          headers: {
            ...request.headers(),
        ...
        // Continue without adding Basic Auth headers for all other requests
        route.continue();
      }
    });
    ...
    await page.goto('./');
    await page.getByRole('button', { name: 'GitLab logo Sign In with GitLab' }).click();
