...
def mock_utils():
    ...
    with patch('src.pkg.users.utils.importYaml') as mi, \
         patch('src.pkg.users.utils.exportYaml') as me, \
         patch('src.pkg.users.utils.replaceAll') as mr:
...
def mock_user_operations():
    ...
    with patch('src.pkg.users.createUserFiles') as mc, \
         patch('src.pkg.users.addUsersToCompose') as ma, \
         patch('src.pkg.users.startUserContainers') as ms, \
         patch('src.pkg.users.stopUserContainers') as mst:
...
def test_add_users_to_compose(mock_utils):
    users.addUsersToCompose(["user1", "user2", "user3"], {"services": {}}, "localhost", "/test")
    assert mock_utils['replace'].call_count == 3


def test_add_users_to_compose_config_error():
    with patch('src.pkg.users.getComposeConfig', return_value=(None, Exception("Error"))):
        assert users.addUsersToCompose(["user1"], {"services": {}}, "localhost", "/test") is not None


@pytest.mark.parametrize("server,file", [("localhost", "users.local.yml"), ("foo.com", "users.server.yml")])
...
def test_get_compose_config_error():
    with patch('src.pkg.users.utils.importYaml', return_value=(None, Exception("Error"))):
        assert isinstance(users.getComposeConfig("testuser", "localhost", "/test"), Exception)
...
def test_run_command_for_containers(mock_run, returncode, has_error):
    mock_run.return_value = MagicMock(returncode=returncode, stderr="Error" if has_error else "")
    assert (users.runCommandForContainers("up", ["user1"]) is not None) == has_error


# addUsers tests
@pytest.mark.parametrize("compose,field", [({"services": {}}, "version"), ({"version": "3"}, "services")])
def test_add_users_missing_fields(mock_config, mock_utils, mock_user_operations, compose, field):
